...
// Only initialize analytics if supported (not on native iOS/Android)
let analytics = null;
isSupported().then(yes => {
  if (yes) {
    ...
    console.log('ℹ️ Firebase Analytics not supported on this platform (normal for native apps)');
  }
}).catch(err => {
  console.log('ℹ️ Firebase Analytics disabled:', err.message);
});
